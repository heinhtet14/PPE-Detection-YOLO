...
def ppe_detection(video_path_param, save_path_param, enable):  
    ...
    while True: 
        ...
        if not (worker_class_track is None):
            data_frames = PPE_DECISION(worker_class_track, object_class, class_names, enable, i_frame=i_frame)
            ...
            if i_frame >= number_pass_frame + 2: 
                ...
                store_workers.pop(0)
                detected_img = PPE_DRAW_DETECTION(frame, worker_class_track, object_class, class_names,enable, visible=False, i_frame=i_frame, frame_average=frame_average)
